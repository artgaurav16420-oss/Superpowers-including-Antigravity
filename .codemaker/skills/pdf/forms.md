...
1. Run the `fill_fillable_fields.py` script from this file's directory to create a filled-in PDF:
...
1. **row_boundaries**: Row top/bottom positions calculated from horizontal lines
...
1. entry x0 = label x1 + 5 (small gap after label)
1. entry x1 = next label's x0, or row boundary
1. entry top = same as label top
1. entry bottom = row boundary line below, or label bottom + row_height

#### Checkboxes

1. Use the checkbox rectangle coordinates directly from form_structure.json
1. entry_bounding_box = [checkbox.x0, checkbox.top, checkbox.x1, checkbox.bottom]
