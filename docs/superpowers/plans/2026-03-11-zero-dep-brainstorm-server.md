...
function startServer() {
  ...
  server.listen(PORT, HOST, () => {
    const info = JSON.stringify({
      ...
      url_host: URL_HOST, url: '[http://']([http://')]([http://'))](http://'))) + URL_HOST + ':' + PORT,
...
- [ ] **Step 2: Open browser to [http://localhost:9876**]([http://localhost:9876**)]([http://localhost:9876**))](http://localhost:9876**)))
