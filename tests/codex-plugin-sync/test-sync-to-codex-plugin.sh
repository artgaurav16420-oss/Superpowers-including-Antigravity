...
EOF
    ...
    cat > "$repo/assets/mega-skills-small.svg" <<'EOF'
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/mega-skills/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/mega-skills/.fixture-keep"
    cat > "$repo/plugins/mega-skills/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/mega-skills/.fixture-keep
    git -C "$repo" add plugins/mega-skills/skills/example/SKILL.md
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/mega-skills/.codex-plugin" \
        "$repo/plugins/mega-skills/.private-journal" \
        "$repo/plugins/mega-skills/assets" \
        "$repo/plugins/mega-skills/skills/example"

    cat > "$repo/plugins/mega-skills/.codex-plugin/plugin.json" <<EOF
{
  "name": "mega-skills",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/mega-skills/assets/mega-skills-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/mega-skills/assets/app-icon.png"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/mega-skills/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/mega-skills/.codex-plugin/plugin.json \
        plugins/mega-skills/assets/app-icon.png \
        plugins/mega-skills/assets/mega-skills-small.svg \
        plugins/mega-skills/skills/example/SKILL.md \
        plugins/mega-skills/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/mega-skills/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/mega-skills/.fixture-keep"
    printf 'stale ignored leak\n' > "$repo/plugins/mega-skills/.private-journal/leak.txt"
    git -C "$repo" add plugins/mega-skills/.fixture-keep
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/mega-skills/skills/example/SKILL.md"
    ...
    assert_contains "$preview_section" "assets/mega-skills-small.svg" "Preview includes SVG asset"
    ...
    assert_not_contains "$preview_output" "Assets (mega-skills-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/mega-skills-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/mega-skills/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/mega-skills" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/mega-skills-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/mega-skills'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/mega-skills-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/mega-skills-*" "Clean no-op local apply does not create sync branch in destination checkout"
