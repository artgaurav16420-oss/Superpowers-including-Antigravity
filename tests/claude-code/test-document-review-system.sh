...
mkdir -p docs/mega-skills/specs

# Create a spec document WITH INTENTIONAL ERRORS for the reviewer to catch
cat > docs/mega-skills/specs/test-feature-design.md <<'EOF'
# Test Feature Design
...
Then review the spec at $TEST_PROJECT/docs/mega-skills/specs/test-feature-design.md using the criteria from that template.
